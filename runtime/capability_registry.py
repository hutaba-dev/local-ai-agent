...
TOOL_SPECS = {
    ...
    "media_generate_image": AgentToolSpec("media_generate_image", "media", "Generate one image from explicit subject, composition, style, camera, lighting, and background intent; optionally save it to the current Project.", "media-mcp", "gpu_compute", "EXECUTE_MEDIA", _object({"subject": _string("Primary visual subject", 500), "intent": _string("Additional visual intent", 2000), "composition": _string("Composition", 300), "style": _string("Visual style", 300), "camera": _string("Camera or framing", 300), "lighting": _string("Lighting", 300), "background": _string("Background", 300), "quality_priority": {"type": "string", "enum": ["FAST", "BALANCED", "QUALITY"]}, "latency_priority": {"type": "string", "enum": ["FAST", "BALANCED"]}, "output_size": {"type": "string", "enum": ["512x512"]}, "save_to_project": {"type": "boolean"}, "source_references": {"type": "array", "items": _string("Research or report result reference", 200), "maxItems": 10}, "related_results": {"type": "object", "additionalProperties": {"type": "string", "maxLength": 2000}, "maxProperties": 10}}, ("subject",))),
    ...
    "google_docs_create": AgentToolSpec("google_docs_create", "google", "Create a native Google Docs document from Markdown headings, emphasis, lists, and tables.", "google-mcp", "external", "WRITE_ARTIFACT", _object({"title": _string("Document title", 300), "content": _string("Structured Markdown report body", 20000), "folder_id": _string("Reserved for future folder placement; omit in this phase", 100)}, ("title", "content"))),
    "google_sheets_create": AgentToolSpec("google_sheets_create", "google", "Create a formatted Google Sheet from typed, sourced tabular data.", "google-mcp", "external", "WRITE_ARTIFACT", _object({"title": _string("Spreadsheet title", 300), "dataset": {"type": "object", "properties": {"title": _string("Dataset title", 300), "columns": {"type": "array", "items": _string("Column name", 300), "minItems": 2, "maxItems": 50}, "rows": {"type": "array", "items": {"type": "array", "items": {"type": ["string", "number", "null"]}, "minItems": 2, "maxItems": 50}, "minItems": 2, "maxItems": 499}, "column_types": {"type": "array", "items": {"type": "string", "enum": ["text", "integer", "decimal", "percent", "currency", "date"]}, "minItems": 2, "maxItems": 50}, "units": {"type": "array", "items": {"type": ["string", "null"]}, "maxItems": 50}, "sources": {"type": "array", "items": _string("Source ID or URL", 500), "minItems": 1, "maxItems": 100}, "chart_candidates": {"type": "array", "items": {"type": "object"}, "maxItems": 4}}, "required": ["title", "columns", "rows", "column_types", "sources"], "additionalProperties": False}, "values": {"type": "array", "items": {"type": "array", "items": {"type": ["string", "number", "boolean", "null"]}, "minItems": 1, "maxItems": 50}, "minItems": 1, "maxItems": 500}, "headers": {"type": "array", "items": {"type": ["string", "number", "boolean", "null"]}, "minItems": 1, "maxItems": 50}, "rows": {"type": "array", "items": {"type": "array", "items": {"type": ["string", "number", "boolean", "null"]}, "minItems": 1, "maxItems": 50}, "maxItems": 499}, "sheet_name": _string("Optional first worksheet name", 100), "start_range": _string("Optional A1 start range; defaults to A1", 100), "folder_id": _string("Reserved for future folder placement; omit in this phase", 100)}, ("title",))),
