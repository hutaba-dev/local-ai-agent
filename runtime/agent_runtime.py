...
from runtime.google_artifacts import TabularDataset, sanitize_report
...
class AgentRuntime:
    ...
    def _run_post_research_orchestration(
        ...
        if google_scope is not None:
            google_tools = {
                ...
                "google_sheets_create": {"name": "google_sheets_create", "purpose": "Create a typed Sheet from genuinely useful sourced metrics extracted by runtime.", "required_arguments": ["title"]},
                "google_sheets_add_chart": {"name": "google_sheets_add_chart", "purpose": "Add one chart from a valid dataset chart candidate.", "required_arguments": []},
            }
        ...
        planner_prompt = {
            ...
            "contract": {
                ...
                "rules": [
                    ...
                    "For Sheets provide only a title; runtime separately extracts and validates the typed dataset from the report.",
                    "Select a chart step only when explicitly requested; runtime will skip it unless extracted numeric data is valid.",
        ...
        for step in steps:
            ...
            if step.tool == "project_save_artifact":
                arguments.update({
                    "content": artifact_report[:12_000],
                    "artifact_type": "report",
                    "description": "Final deep-research report generated by KIM",
                })
            elif step.tool == "google_docs_create":
                arguments["content"] = artifact_report[:20_000]
            elif step.tool == "google_sheets_create":
                try:
                    dataset_value = arguments.get("dataset")
                    dataset = (
                        TabularDataset.from_mapping(dataset_value)
                        if dataset_value is not None
                        else self._extract_tabular_dataset(request, artifact_report, latency)
                    )
                except (httpx.HTTPError, ValueError):
                    step_state = {
                        "step_id": step.id, "capability": "google", "tool": step.tool,
                        "status": "INVALID_DATASET", "dependencies": list(step.dependencies),
                        "result_reference": None, "artifact_ids": [], "external_urls": [],
                        "error_category": "INVALID_DATASET", "retryable": True,
                    }
                    state[step.id] = step_state
                    activity.append(self._orchestration_activity(step_state, False, None))
                    continue
                arguments = {"title": str(arguments.get("title") or dataset.title), "dataset": self._dataset_mapping(dataset)}
                sheet_datasets[step.id] = dataset
                ...
                if not isinstance(spreadsheet_id, str) or not spreadsheet_id:
                    ...
                    activity.append(self._orchestration_activity(step_state, False, None))
                    continue
                sheet_step_id = str(sheet_result.get("step_id"))
                dataset = sheet_datasets.get(sheet_step_id)
                candidate = dataset.valid_chart() if dataset else None
                if dataset is None or candidate is None:
                    step_state = {
                        "step_id": step.id, "capability": "google", "tool": step.tool,
                        "status": "NO_VALID_CHART_DATA", "dependencies": list(step.dependencies),
                        "result_reference": None, "artifact_ids": [], "external_urls": [],
                        "error_category": "NO_VALID_CHART_DATA", "retryable": False,
                    }
                    state[step.id] = step_state
                    activity.append(self._orchestration_activity(step_state, False, None))
                    continue
                end_column = len(candidate.series_columns) + 1
                column_name = "".join(reversed([
                    chr(65 + digit) for digit in self._spreadsheet_column_digits(end_column)
                ]))
                arguments = {
                    "spreadsheet_id": spreadsheet_id,
                    "chart_type": candidate.chart_type,
                    "data_range": f"A1:{column_name}{len(dataset.rows) + 1}",
    ...
    def _parse_orchestration_steps(content: str, allowed_tools: set[str]) -> tuple[OrchestrationStep, ...]:
        ...
        for raw in raw_steps:
            ...
            if tool == "google_sheets_create" and arguments.get("dataset") is not None:
                TabularDataset.from_mapping(arguments.get("dataset"))
    ...
    def _extract_tabular_dataset(
        self, request: str, report: str, latency: LatencyRecorder,
    ) -> TabularDataset:
        schema = {
            "dataset": {
                "title": "string", "columns": ["string"], "rows": [["typed scalar or null"]],
                "column_types": ["text|integer|decimal|percent|currency|date"],
                "units": ["string or null"], "sources": ["source ID or URL"],
                "chart_candidates": [{
                    "title": "specific title", "chart_type": "LINE|BAR|COLUMN|PIE",
                    "category_column": 0, "series_columns": [1],
                }],
            }
        }
        system = (
            "Extract one compact, useful TabularDataset from the supplied completed report. Return JSON only. "
            "Use only values explicitly present in the report and preserve their scope and unit. Never estimate, infer, or invent numbers. "
            "Numeric columns contain JSON numbers, not formatted strings; unknowns are null, never zero. "
            "Column 0 must be a text category. Put contiguous comparable numeric series next, then a separate Evidence column containing source IDs or URLs. "
            "Use at least two rows. If a numeric series has fewer than two sourced values, omit chart_candidates. "
            "Percent values are decimal fractions. Do not combine unlike units or time periods into one numeric series."
        )
        source = json.dumps({
            "request": request, "completed_report": report[:16_000], "output_schema": schema,
        }, ensure_ascii=False)
        content, _ = self._complete(
            [{"role": "system", "content": system}, {"role": "user", "content": source}],
            3200, latency, "tabular_dataset_extraction", temperature=0,
        )
        try:
            value = self._parse_json_object(content)
            return TabularDataset.from_mapping(value.get("dataset"))
        except ValueError as error:
            repaired, _ = self._complete(
                [{"role": "system", "content": (
                    system + " Repair the candidate to satisfy the schema using only values already present in the candidate or report. "
                    "Do not add rows, sources, or numeric values."
                )}, {"role": "user", "content": json.dumps({
                    "source": source, "invalid_candidate": content[:12_000],
                    "validation_error": str(error),
                }, ensure_ascii=False)}],
                3200, latency, "tabular_dataset_repair", temperature=0,
            )
            value = self._parse_json_object(repaired)
            return TabularDataset.from_mapping(value.get("dataset"))

    @staticmethod
    def _dataset_mapping(dataset: TabularDataset) -> dict[str, object]:
        return {
            "title": dataset.title,
            ...
            "chart_candidates": [{
                "title": item.title, "chart_type": item.chart_type,
                "category_column": item.category_column, "series_columns": list(item.series_columns),
            } for item in dataset.chart_candidates],
        }

    @staticmethod
    def _spreadsheet_column_digits(column_number: int) -> list[int]:
        digits: list[int] = []
        while column_number:
